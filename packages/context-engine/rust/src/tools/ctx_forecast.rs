...
pub fn handle(task: &str, files: &[String], model_filter: Option<&str>) -> String {
    ...
    // Complexity
    out.push(format!(
        "Complexity: {}  (multiplier: {:.1}x)",
        complexity.label(),
        multiplier
    ));
    if !file_details.is_empty() {
        out.push(format!(
            "Files: {} ({} estimated tokens raw)",
            files.len(),
            format_tokens(base_tokens as u64)
        ));
        ...
        out.push(format!(
            "Estimated scope: {} tokens (from task description)",
            format_tokens(raw_tokens as u64)
        ));
    }
    ...
    out.push("─".repeat(40));
    out.push(format!(
        "  Without LoopGuard  input: {:>8}  output: {:>7}",
        format_tokens(estimated_input),
        format_tokens(estimated_output)
    ));
    out.push(format!(
        "  With LoopGuard     input: {:>8}  output: {:>7}  (focused slice ~20%)",
        format_tokens(focused_input),
        format_tokens(focused_output)
    ));
    ...
    let target_models: Vec<&ModelPricing> = if let Some(filter) = model_filter {
        MODELS
            .iter()
            .filter(|m| m.name.to_lowercase().contains(&filter.to_lowercase()))
            .collect()
    } else {
        MODELS.iter().collect()
    };
    ...
    for m in &target_models {
        ...
        let saving_pct = if cost_without > 0.0 {
    ...
    if focused_input > 40_000 {
        ...
        out.push(
            "✓  Medium session. ctx_read with 'map' mode for large files to keep context lean."
                .to_string(),
        );
    } else {
        out.push(
            "✓  Small session. Proceed. ctx_compress at midpoint if context grows.".to_string(),
        );
    }
    ...
    if let Some((model, cost_val)) = cheapest_with {
        if cost_val < 0.01 {
            out.push(format!(
                "   Cheapest option: {model} at ${cost_val:.4} with LoopGuard focused reads."
            ));
        }
    }
