...
pub fn exec(command: &str) -> i32 {
    ...
    let child = match child {
        Ok(c) => c,
        Err(e) => {
            eprintln!("loopguard-ctx: failed to execute: {e}");
            return 127;
        }
    };
    ...
    let output = match child.wait_with_output() {
        Ok(o) => o,
        Err(e) => {
            eprintln!("loopguard-ctx: failed to wait: {e}");
            return 127;
        }
    };
    ...
    if cfg.tee_on_error && exit_code != 0 && !full_output.trim().is_empty() {
        if let Some(path) = save_tee(command, &full_output) {
            eprintln!(
                "[loopguard-ctx: output saved to {path} (secrets redacted, auto-deleted after 24h)]"
            );
        }
    }
...
pub fn interactive() {
    ...
    eprintln!("loopguard-ctx shell v2.6.0 (wrapping {real_shell})");
    ...
    loop {
        let _ = write!(stdout, "loopguard-ctx> ");
        let _ = stdout.flush();
    ...
    if let Some(compressed) = patterns::compress_output(command, output) {
        if !compressed.trim().is_empty() {
            ...
            if compressed_tokens < original_tokens {
                ...
                return format!(
                    "{compressed}\n[loopguard-ctx: {original_tokens}→{compressed_tokens} tok, -{pct}%]"
                );
            }
        }
    }
...
fn detect_shell() -> String {
    if let Ok(shell) = std::env::var("LOOPGUARD_CTX_SHELL") {
        return shell;
    }
...
fn save_tee(command: &str, output: &str) -> Option<String> {
    let tee_dir = dirs::home_dir()?.join(".loopguard-ctx").join("tee");
