...
fn slow_log_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".loopguard-ctx").join(LOG_FILENAME))
}
...
pub fn list() -> String {
    ...
    match std::fs::read_to_string(&path) {
        Ok(content) if !content.trim().is_empty() => {
            ...
            let header = format!(
                "Slow command log ({} entries)  [~/.loopguard-ctx/{}]\n{}\n",
                lines.len(),
                LOG_FILENAME,
                "─".repeat(72)
            );
        ...
        Err(_) => format!("No slow log found at ~/.loopguard-ctx/{LOG_FILENAME}"),
    }
}
...
pub fn clear() -> String {
    ...
    match std::fs::remove_file(&path) {
        Ok(()) => format!("Cleared ~/.loopguard-ctx/{LOG_FILENAME}"),
