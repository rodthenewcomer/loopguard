...
fn stats_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".loopguard-ctx"))
}
...
fn load_mcp_live() -> Option<serde_json::Value> {
    let path = dirs::home_dir()?.join(".loopguard-ctx/mcp-live.json");
    let content = std::fs::read_to_string(path).ok()?;
...
pub fn format_cep_report() -> String {
    ...
    if total_saved == 0 && cep.modes.is_empty() {
        ...
        o.push(format!(
            "     Run {CYAN}loopguard-ctx setup{RST} to auto-configure your editors."
        ));
        o.push(String::new());
    }
    ...
    if avg_score >= 70.0 {
        o.push(format!(
            "    {GREEN}✓{RST} Great score! You're using loopguard-ctx effectively"
        ));
    }
    o.push(String::new());
...
pub fn format_gain() -> String {
    ...
    if store.total_commands == 0 {
        return format!("{DIM}No commands recorded yet.{RST} Use {CYAN}loopguard-ctx -c \"command\"{RST} to start tracking.");
    }
    ...
    let ln56 = line('─', 56);
    o.push(format!(
        "  {BOLD}{WHITE}◆ loopguard-ctx{RST}  {DIM}Token Savings Dashboard{RST}"
    ));
    o.push(format!("  {DIM}{ln56}{RST}"));
    o.push(String::new());
    ...
    o.push(format!("  {DIM}{ln56}{RST}"));
    o.push(format!(
        "  {DIM}loopguard-ctx v{}  |  loopguard.vercel.app  |  loopguard-ctx dashboard{RST}",
...
pub fn format_gain_graph() -> String {
    ...
    if store.daily.is_empty() {
        return format!(
            "{DIM}No daily data yet.{RST} Use loopguard-ctx for a few days to see the graph."
        );
    }
    ...
    let ln58 = line('─', 58);
    o.push(format!(
        "  {BOLD}{WHITE}◆ loopguard-ctx{RST}  {DIM}Token Savings Graph (last 30 days){RST}"
    ));
...
pub fn format_gain_daily() -> String {
    ...
    let lnw = line('─', w);
    o.push(format!(
        "  {BOLD}{WHITE}◆ loopguard-ctx{RST}  {DIM}Daily Breakdown{RST}"
    ));
